...
func TestMigration0033_TaskWorkingStateShape(t *testing.T) {
	...
	for _, e := range entries {
		...
		if v > 33 && v != 34 && v != 35 && v != 36 && v != 37 && v != 38 && v != 39 && v != 40 && v != 41 && v != 42 && v != 43 {
			t.Errorf("migrations/%s exists: criterion 17 — 0033 is the only migration this ticket adds and none above it exists except a number another ticket owns (34: chat-on-closed-task, 35: treetop-pr-review-tasks, 36: signal-session-name, 37: microsoft-oauth-mail, 38: gmail-delivery-cc, 39: activity-resurfaces, 40: comms-inbox, 41: slack-watch-sweep, 42: slack-send-queue, 43: slack-channel-mentions)", e.Name())
		}
	}
