...
- Client email replies: any session can draft one with draft_delivery (channel gmail, with the thread_id of the mail being answered) and fix its own drafts with update_delivery. Salvador approves and sends on the dashboard, which shows From and To first; a draft is not sent, so never say it was.
