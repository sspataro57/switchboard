...
- "swb start <id>", or starting work on a swb task in this session → call task_signal with state working, always with session = this session's name, from the first line of ListAgents (` + "`This session is <name>`" + `; the name only). Just before you stop to wait on Salvador's answer, call it with needs_input and the session, and with working again when he replies: he answers in this session's console, never through switchboard, which records only the state and the session name. "swb stop <id>" → task_signal with clear. Finishing is task_close, which also clears it. Only for a human task you are working on in this conversation.
