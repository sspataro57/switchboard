...
**Image:** `192.168.50.20:5000/switchboard:0.7.16` (`sha256:364c0b88554b1ac6bdfeb842e98d6d16356fff5d10c7cddb95212d5fc51d76dd`, main 75fe2c1; handed off 2026-09-13). One step at a time; steps 4 and 5 are not kube steps and are listed so the order is visible.
...
4. **Candidates are seeded by the switchboard session, through opsctl** (not a kube step): `opsctl route-candidates add --account salvador@handsonconnect.org --provider google --project collaboratory --default --description "…"` and the same for `reengine` (O2, O3). `--provider google` is required: that address is a `google`, a `jira` and a `jira_lookup` source account, so without it the add is refused as ambiguous, and the mailbox being routed is the `google` one (SPEC B8 amendment 2026-09-13). The flag needs an opsctl built from main after that amendment. Every add is an audited executor call as `opsctl:$USER`. Until an account has candidate rows, the route stage finds nothing to do.
