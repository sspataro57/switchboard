...
| **pipelined** | **Deployment (new)**, `replicas: 1`, `strategy: Recreate` | command `/usr/local/bin/pipelined`; env below; no ports, no Service |
...
4. `kubectl -n ops delete pod -l app=pipelined --grace-period=0 --force` shows `{"state":"dead"}` on `ops/workers/pipeline.daemon/status`, then `idle` again once the new pod connects.
