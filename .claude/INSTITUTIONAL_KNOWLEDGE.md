...
- **The close record, and its fallback.** `closeTransition`, the one writer of
  ...
  makes the revive guard fall back to `updated_at`, the task's last STAMPED
  write. On a closed task only closeTransition stamps it (`task_append_log`
  and `task_mark_surfaced` do not; a capture-pass integration test proves the
  log-then-revive order leaves it alone), so for any close made through the
  executor it IS the close instant. It is NOT "conservative only": it errs
  both ways when something outside the executor wrote the row. A hand-run
  UPDATE of `updated_at` after the close moves the guard later, so a message
  ingested in between does not revive. A close written without stamping it
  (hand SQL `SET status='closed'`, a fixture INSERT with an old `updated_at`)
  leaves the guard BEFORE the real close, so a message ingested in between
  revives although it predates the close. The fallback is spelled once, in
  `reviveGuarded`'s SQL.
...
- **The own-action guard (J17) is a backstop for his own COMMENTS only.** With
  Jira's "Notify me about my own changes" on, Jira mails him ("Anonymous
  (JIRA)") about his own changes. That mail is inbound and would revive, or
  create, a task for his own action. Capture skips an activity revive or
  creation when the ticket's connector thread (`jira:{site_host}:{KEY}`) holds
  an OUTBOUND message sent in [email − 10m, email + 2m]. Those are
  `OwnActionLead`/`OwnActionLag` in `internal/capture/ownaction.go`.
  - **LANDMINE: it does not cover his EDITS.** Prod has 208 Anonymous Treetop
    emails; only 26 correlate with a comment of his. The other 182 are
    field/status-edit notices (146 have no outbound message even in
    [−60m, +10m]), and the connector stores no changelog (0 of 2,431 issue raw
    rows), so nothing names an edit's actor. Those revive or create. The only
    protection against his own edits is the owner turning the Jira setting off.
    Do NOT "fix" this with an Anonymous-sender suppression: 182 Anonymous
    emails that are not his comments would then be dropped on a guess.
  - **Named-actor exemption.** A From of Jira's shape `"<Name> (JIRA)"` whose
    Name is neither the placeholder `Anonymous` nor the stored sender of the
    outbound message the window found is SOMEONE ELSE's action: it proceeds at
    once, with no window skip, no freshness reads and no wait (reason `actor
    named`). Prod email 158474, "Katie Evans mentioned you on WEB-10355",
    arrived 8.6 min after his own comment on that ticket, and the window alone
    would have suppressed it. The name check only EXEMPTS. The placeholder
    never suppresses anything by itself.
  - Residual: an Anonymous email about someone else's action, inside the
    window around his own comment, is still suppressed.
  - The revive path logs and records `own_action … revive skipped`. The create
    path records `attributed … no task created`, so no lower rule takes the
    message.
  - The race: a key's poller is any provider='jira' account whose scopes claim
    its prefix. The thread is known synced when EVERY such poller has an `ok`
    sync_run that STARTED after email + 4m and holds no unnormalized raw row of
    the key. Until then the message is DEFERRED: no capture_decisions row, a
    `capture rules: message N deferred` log line, `RulesStats.Deferred`, and a
    retry every pass. The deferral lasts at most 30 min from ingest
    (`OwnActionMaxWait`); after that the pass decides with the guard BLIND
    (fail-open), the reason says so, and `RulesStats.Blind` counts it. Every
    capture counter line prints `"deferred"` and `"blind"`.
  - **Live horizon floor.** A deferral writes no decision row, so a live
    horizon shorter than the deferral could let a message age out undecided.
    `RulesConfig.normalize` refuses a LIVE horizon below `MinLiveRulesHorizon`
    (2h). A positive `CAPTURE_RULES_SINCE` / `--since` under 2h now ERRORS
    every live pass. Garbage or non-positive values still fall back silently
    to 720h, unchanged. Prod leaves `CAPTURE_RULES_SINCE` unset.
  - `opsctl capture-rules run --limit N` reads the oldest N first, so deferred
    rows can use up the limit for up to 30 min. `--limit` is smoke-only.
  - A key no poller covers (lookup-only reengine/LHH) is "not applicable":
    nothing stores his comments there. The owner turning the Jira setting off
    is the primary fix; the guard is the backstop.
  - The cost: someone ELSE's activity on the same ticket inside that window
    around his own comment does not revive either.
- **LANDMINE (rollout order, 0030; docs/runbooks/HANDOFF-kube-jira-activity-revive.md).**
  1. Apply 0030 FIRST (kube one-shot migrate Job). New code selects
     `capture_rules.revive` on every capture pass and writes `tasks.closed_at`
     on every close, so a new image on a db without 0030 fails both.
  2. Roll ONE image tag to every connector CronJob and pipelined together. A new
     capture that revives, running beside an old jira-image reconciler that
     ignores surfacing, re-closes the task: one flip per message until the
     images match.
  3. Then orchestratord and the dashboard (they call `task_close`; until rolled
     their closes carry `closed_at` NULL, which the fallback covers).
  4. `go install ./cmd/ops-mcp-user` here and on 192.168.50.30.
  5. Seed revive rules only after EVERY workload runs the new image. An old
     capture binary does not read the flags: it would take J2 as a plain
     priority-92 rule, creating unsurfaced per-ticket tasks and logging on
     closed ones without reviving.
  6. Seed J2 only after the owner has turned off Jira's "Notify me about my own
     changes" AND the SPEC's 0c gate passes. That gate is an UNCORRELATED count
     of Anonymous Treetop mail received after the setting change, observed for
     a full working day or more. 0 → seed. Any → STOP and ask the owner, who
     decides an exclusion in the rule's DATA. The sender predicate is
     `ILIKE '%anonymous (jira)%'`: prod stores the From QUOTED, so
     `'anonymous%'` never matches.
     J4 is not seeded (0 Avviato addressed shapes on prod).
- **capture_rule_add refuses `revive` unless `external_system='jira'`.** Part
  D's hold keys on `system == "jira"`, so a reviving github/slack/gmail rule on
...
A `capture_rules.pr_review` rule makes ONE human `ready` task per treetopllc PR he
did not author, from the GitHub notification mail he already receives. Runbook:
`docs/runbooks/capture-rules.md` "PR review rules (SWT-54)". Deploy:
`docs/runbooks/HANDOFF-kube-treetop-pr-review-tasks.md`.

- **One PR key spelling: `{owner}/{repo}#{N}`, via `github.PRKey`.** The seeded
  key_regex captures the thread root's PATH (`…/pull/{N}`), and the driver
  canonicalizes EVERY github-derived key through `github.ParsePRRef` + `PRKey`. A
  github key that does not parse is attribution only. `PGTaskResolver.Resolve`
  uses `PRKey` too, and a structure test fails on any other `"%s#%d"` key under
  `internal/`. Two spellings would give one PR two tasks, silently (the
  SWT-13/18/19 landmine).
- **Authorship is read from the RAW row, never a normalized column.**
  `normalized_messages` carries no X-GitHub-* header.
  - `google.RawMailHeaders` decodes `raw_json.rfc822_b64` headers only, and
    `ok=false` for a non-IMAP row.
  - `github.NotificationFacts` reads them through `mail.Header.Get`: the map is
    keyed `X-Github-Reason`, so indexing it with `X-GitHub-Reason` finds nothing.
  - `decidePRAuthor` is pure.
  - Verdict `own`/`excluded` falls through: the message is re-decided without the
    rule, so his own PR's mail keeps today's decision.
  - `undetermined` creates (fail-open).
  - His login is data (`X-GitHub-Recipient`), never a literal.
- **A pr_review rule acts only on mail the receiving MX authenticated as
  GitHub's** (2026-09-14, Codex CRITICAL). The thread key and every X-GitHub-*
  header are sender-controlled, so a forged mail could create, suppress or close a
  review task.
  - `trustedGitHubNotification` reads ONLY the TOPMOST `Authentication-Results`.
    Gmail prepends its own, and anything below it came with the message.
  - It needs authserv-id exactly `mx.google.com` (`prTrustedAuthServID`) and
    `dkim=pass` with `header.d=github.com` or `header.i=…@github.com`. Prod has this
    on 121 of 121 PR-thread mails, on both accounts.
  - An untrusted mail falls through like his own PR's mail, with the reason
    `untrusted GitHub mail`. Authorship evidence counts trusted mails only.
  - A `{}` (non-IMAP) row is untrusted.
  - **dkim=pass is not enough on its own: headers must be single-instance.** A
    relayed genuine GitHub mail still passes DKIM with unsigned copies PREPENDED,
    and `mail.Header.Get` returns the FIRST (attacker's) copy. So a duplicated
    `Message-ID`, `References`, `In-Reply-To` or X-GitHub-* header makes the mail
    untrusted (`duplicatedGitHubHeader`); the reason names the header. The body
    notice is covered by DKIM's body hash (no `l=`). Unmodified relays of an
    attacker's own GitHub mail are harmless: the root names their repo.
  - **GitHub does NOT DKIM-sign `Message-ID` or any `X-GitHub-*` header.** This
    was checked on prod `h=` lists, 2026-09-14. Openings sign `subject` and
    `list-id` but no thread headers; follow-ups also sign `in-reply-to` and
    `references`. So a relayed genuine opening of the attacker's own PR with a
    REPLACED Message-ID would retarget the thread key.
    - The PR is therefore bound to SIGNED headers (`prMailBindingFailure`):
      - `List-ID`'s angle part must be `{repo}.{owner}.github.com`. Prod has
        exactly one List-ID per mail, and 123 of 123 are
        `owner/repo <repo.owner.github.com>`.
      - The RFC 2047-DECODED `Subject` must end `(PR #N)` (123 of 123 once
        decoded; one prod subject is encoded-word).
    - A mismatch is untrusted, and the reason names the binding.
    - Accepted residual: a relayer of GENUINE treetopllc PR mail can still spoof
      the unsigned X-GitHub-Reason/Sender/Recipient. At most one review task is
      suppressed.
  - If the mailboxes ever leave Gmail, change the one constant. If `MAIL_SOURCE`
    moves off `imap` (bridge, gmail_api), no RFC822 is stored and EVERY PR mail
    becomes untrusted: the rule silently stops creating tasks.
  - A `"` anywhere in a dkim result is untrusted (a quoted `header.i` could
    smuggle a `header.d=github.com` token; Gmail's real header has no quotes).
  - Outside the model: a delivery path where Gmail does not prepend its own header
    (e.g. intra-Workspace). Accepted: an insider who receives genuine treetopllc
    mail could relay it with an ADDED (not duplicated) X-GitHub-* header; at most
    one review task suppressed.
  - Never add a github-keyed rule WITHOUT pr_review on the same threads: it keys
    the same PR, and on an untrusted fall-through its empty notice flag would let
    SWT-36 reopen a dismissed review task.
- **The PR state notice is the FIRST non-empty body line only**
  (`github.PRStateNotice`: merged, closed or reopened). "merged in #3202" prose in PR
  descriptions is real on prod.
  - Merged/closed on an open review task closes it through `task_close` as
    `capture:{connector}`; an active-work refusal is a skip.
  - Merged/closed as the first mail seen creates nothing.
  - `Reopened #N.` only logs; as a first mail it creates the task.
  - **No state notice ever reopens a DISMISSED review task** (owner decision
    2026-09-14): the guard is in `decideMessage`'s found branch
    (`dismissalID != 0 && d.prNotice == ""`), so live, shadow and `try` agree.
- **A `col = 'x'` inside a CHECK passes on NULL.** 0035's first draft,
  `NOT pr_review OR (external_system = 'github' AND …)`, let `pr_review=true` with a
  NULL system through: NULL AND … is NULL, false OR NULL is NULL, and a CHECK only
  refuses FALSE. Spell `col IS NOT NULL AND col = 'x'` when the CHECK must fail
  closed.
- **Rollout barrier (procedural).** Apply 0035 first: `loadRules` selects the
  columns on every pass.
  - Seed the rule only after `kubectl -n ops get cronjob,deploy -o wide` shows the
    new image on EVERY capture workload.
  - An old binary takes a pr_review rule as a plain github rule: path-spelled refs,
    tasks for his own PRs, and later a second task per PR.
  - Never roll an image back past SWT-54 while a pr_review rule is enabled. Disable
    it first with `capture_rule_set_enabled`.
- **`opsctl capture-rules try` is the write-nothing dry run for any candidate rule**
  (`capture.DryRunRules`, the same `decideMessage`). It is how a rule is proven on
  the real corpus before F8 makes it permanent, and it prints the one-off D9
  backfill payloads.
  - **Save the PRE-add output.** After the add, `try` with the same (project, type,
    pattern) REFUSES by name, because the candidate could never win against the
    stored rule and would print an empty backfill.
  - `try` also refuses `--revive`/`--addressed`: it does not simulate activity
    rules.
- **`url_template` is refused on github rules**: `{key}` would contain `#`.
- **Accepted residuals:**
  - **A task can occasionally appear for HIS OWN PR.** Undetermined authorship
    is fail-open by design: a wrong task costs one close, a missed review is
    invisible. Prod: 2 of 55 PRs in 90 days were undetermined, both someone
    else's. It happens when his first mail on his own PR says reason `mention`,
    or when the `author` mail went to the other account.
    - **Close it as Done (`task_close`).** A Dismiss does not stick: SWT-36
      reopens a dismissed task on the PR's next trusted mail that is not a merged, closed or reopened notice.
  - A transient `link_external_ref` failure after `create_task` can give one PR
    a second task. This is the general capture partial-write weakness, tracked in
    SWT-50.
  - two receiving accounts (0f), and the thread-side read covers one of them;
...
- **Cross-ticket rule (SWT-54 D4, added at the SWT-53/SWT-54 merge): a GitHub PR
  state notice never resurfaces.** `resurfaceInput.prNotice` (set from
  `decideMessage`'s `d.prNotice`, i.e. a TRUSTED merged/closed/reopened notice on
  a pr_review rule's PR) is its own disqualifier with its own reason (`PR state
  notice ... SWT-54 D4`). It holds even with an empty notifier list, so the
  notice that closed a review task can never bring it back through the inquiry
  lane. An ordinary comment on a CLOSED review task is a `task_log` that the
  GitHub seed (`notifications@github.com`) keeps silent; with an empty list it
  resurfaces like any human message. An UNTRUSTED notice falls through before any
  pr_review action (no notice flag) to rule 6 (attributed) or rule 10's bucket,
  where the same seed keeps it silent. Residual: his OWN PR's notice falls
  through to rule 10 and is re-decided without the flag, so on a closed bucket
  only the seed keeps it silent.
- **K6 is about From PARSERS, not the `net/mail` import.** SWT-54's
  `prreview.go` / `prreview_store.go` use `mail.Header` (the stored RFC822 header
  map) and are allowed; `mail.ParseAddress` / `mail.AddressParser` live only in
  `senderdomain.go` (`TestSenderAddress_LivesBesideSenderDomainAndIsItsParser`).
