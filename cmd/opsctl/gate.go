...
func printGateStats(mode string, st capture.GateStats) {
	const revived, surfacedCreated, surfacedOpen, deferred, blind, resurfaced, prAuthorSkipped, prClosed, commTasks = 0, 0, 0, 0, 0, 0, 0, 0, 0
	fmt.Printf("capture_gate: {\"mode\":%q,\"tasks_created\":%d,\"appended\":%d,\"reopened\":%d,"+
		"\"revived\":%d,\"surfaced_created\":%d,\"surfaced_open\":%d,\"deferred\":%d,\"blind\":%d,\"resurfaced\":%d,"+
		"\"pr_author_skipped\":%d,\"pr_closed\":%d,\"comm_tasks\":%d,"+
		"\"attributed\":%d,\"pending_lookup\":%d,\"budget_skipped\":%d,\"resolved\":%d}\n",
		mode, st.TasksCreated, st.Appended, st.Reopened, revived, surfacedCreated, surfacedOpen, deferred, blind, resurfaced,
